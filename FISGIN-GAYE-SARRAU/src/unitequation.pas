...
for i:=1 to length(equation) do
	...
	else if (equation[i] = 'x') and (equation[i+1] <> '^') and (equation[i+1] = '+') or (equation[i+1] = '-') then
		begin
...
while (i <= Length(equation)) do
	...
	if (equation[i] = '=') then
		begin
		j := i + 1;
		while (j <= Length(equation)) and ((equation[j] <> '+') and (equation[j] <> '-') ) do
		begin
     Inc(j);
		end;
		a := StrToFloat(Copy(equation, i + 1, j-2-i));
		if (j <= Length(equation)) then
			b := StrToFloat(Copy(equation, j + 1, Length(equation) +1 - j))
		else
			b := 0; 
		Break; 
		end;
