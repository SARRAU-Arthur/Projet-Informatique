     ...
     '6':
          begin
            ...
            begin
              ...
              writeln('La régression linéaire est de la forme : Y = ', a, 'X + ', b);
              writeln('Le coefficient de corrélation R^2 est : ', r_squared);
            end
            else
...
begin
nm:=parcoursfichier(currentfile,'stats');
...
var
  n, i: Integer;
  sum_x, sum_y, sum_xy, sum_x_squared, sum_y_squared: Real;
  x_bar, y_bar, numerator, denominator: Real;
begin
  n := Length(TabI);
  sum_x := 0;
  sum_y := 0;
  ...
  for i := 0 to n - 1 do
  begin
    sum_x := sum_x + TabI[i][0];
    sum_y := sum_y + TabI[i][1];
    sum_xy := sum_xy + (TabI[i][0] * TabI[i][1]);
    sum_x_squared := sum_x_squared + Power(TabI[i][0], 2);
    sum_y_squared := sum_y_squared + Power(TabI[i][1], 2);
  end;
  ...
  denominator := (n * sum_x_squared) - Power(sum_x, 2);
  ...
  begin
    ...
    r_squared := Power(numerator, 2) / ((n * sum_x_squared - Power(sum_x, 2)) * (n * sum_y_squared - Power(sum_y, 2)));
  end
  else
  begin
    a := 0;
